...
int	main(int argc, char **argv) {
	...
	if (fstream_setup(infile, ifs, ofs) == false) return 1;
	else
...
int	is_input_good(int argc, char **argv) {
	if (argc != 4)
		return std::cerr << BAD_ARG_COUNT << std::endl, false;
	for (int i = 0; i  < argc; ++i)
		if (std::string(argv[i]).empty())
			return std::cerr << EMPTY_ARG << std::endl, false;
	return (true);
}
...
int	fstream_setup(std::string infile, std::ifstream &ifs, std::ofstream &ofs) {
	...
	if (ifs.is_open() == false)
		return std::cerr << INFILE_ERROR << infile << std::endl, false;
	ofs.open(infile.append(".replaced"));
	if (ifs.is_open() == false)
		return std::cerr << OUTFILE_ERROR << infile << std::endl, false;
	return true;
}
